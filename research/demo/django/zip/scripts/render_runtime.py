...
INTEGER_FIELDS = {
    "workers": (1, 256),
    "threads": (1, 256),
    "worker_connections": (1, 1_000_000),
    "timeout": (1, 3600),
    "keepalive": (0, 3600),
}
...
def normalize(mode: str, environ: dict[str, str] | None = None) -> dict[str, object]:
    ...
    if mode == "custom":
        values["runtime_label"] = environ.get("RUNTIME_LABEL", "").strip()
        values["preset"] = "custom"
        required = ("RUNTIME_LABEL", "GUNICORN_WORKER_CLASS")
    ...
    if not LABEL_RE.fullmatch(label):
        raise ValueError(
            "runtime label must be non-empty and contain only safe characters"
        )
